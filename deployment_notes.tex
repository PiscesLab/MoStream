...
\begin{itemize}
  \item \texttt{NPMMModel.py}: \texttt{skip\_pretrained = True} --- pretrained weights from
        \texttt{model.h5} are incompatible (saved with nfp~0.0.x/TF1, EdgeUpdate had 10 weights;
        current nfp~0.1.3 has 4). Model trains from random init via streaming data.
        Ask professor for the original nfp version or a regenerated \texttt{model.h5}.
