...
@python_app
def SimulationTask(smiles, count):
        ...
        return smiles, data.oxidation_potential['xtb-vacuum'], count    
...
while (len(unsearched) > 0):
        inchi = random.choice(unsearched) 
        smiles_tmp = smiles_list[inchi_list.index(inchi)]
        future = SimulationTask(smiles_tmp, count)
        smiles, ip_simulate, count = future.result()
